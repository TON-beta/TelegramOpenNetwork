...
- [Development guidelines](#development-guidelines)
    - [General ton Core](#general-ton-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
ton Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play tons" on the test network, if you
...
ton Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ton/ton/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ton
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ton --log_level=test_suite
$ valgrind -v --leak-check=full src/tond -printtoconsole
```
...
# A coverage report will now be accessible at `./test_ton.coverage/index.html`.
```
...
To profile a running tond process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep tond` -- sleep 60
```
...
ton Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/ton/ton/issues/12691)
...
However, in open source software such as ton Core, where everyone uses
...
pay attention to for reviewers of ton Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: ton Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `ton_FOO_BAR_H`, e.g.

```c++
#ifndef ton_FOO_BAR_H
#define ton_FOO_BAR_H
...
#endif // ton_FOO_BAR_H
```
...
Some of these are maintained by active developers of ton Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against ton Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/ton-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/ton-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/ton-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors, it will cause problems with ton's `select()` loop, because
...
compatibility between nodes. This happened in ton 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to ton's
...
Commit [`bb81e173`](https://github.com/ton/ton/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ton-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ton-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of ton Core, the wallet was always
