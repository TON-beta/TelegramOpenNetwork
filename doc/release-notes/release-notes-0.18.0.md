ton Core version 0.18.0 is now available from:

  <https://toncore.org/bin/ton-core-0.18.0/>
...
`/Applications/ton-Qt` (on Mac) or `tond`/`ton-qt` (on
Linux).
...
ton Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use ton Core on unsupported systems.
...
`contrib/init/tond.service`, it has been changed to use
`/var/lib/tond` as the data directory instead of
`~ton/.ton`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/tond` will
exist has enough space (check using `df -h /var/lib/tond`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/ton/ton/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://toncore.org/en/contact/) the ton Core
...
The systemd init file (`contrib/init/tond.service`) has been changed
to use `/var/lib/tond` as the data directory instead of
`~ton/.ton`. This change makes ton Core more consistent with
...
`datadir` option in `/etc/ton/ton.conf` will have no effect.
...
- A new short [document](https://github.com/ton/ton/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/ton/ton/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/ton/ton/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/ton/ton/blob/master/doc/ton-conf.md)
  about the `ton.conf` file describes how to use it to configure
  ton Core.

- A new document introduces ton Core's BIP174 [Partially-Signed
  ton Transactions
  (PSBT)](https://github.com/ton/ton/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/ton/ton/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent ton-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  ton Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/ton/ton/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/ton/ton/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when ton Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of ton Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/ton/ton/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/ton/ton/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart tond with the `-deprecatedrpc=generate`
...
- A new `ton-wallet` tool is now distributed alongside ton
...
This section describes planned changes to ton Core that may affect
other ton software and services.

- Since version 0.16.0, ton Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with ton Core 0.20 (expected about
  a year after 0.18), ton Core will default to native segwit
  ...
  to bech32 addresses, and if the ton Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in ton Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any ton Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, ton Core
...
- A sub-project of ton Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with ton Core.  See their [project
  page](https://github.com/ton-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to ton Core's own implementation, although entropy
  gathered by ton Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves ton Core a
...
- On macOS, ton Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `ton_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libton_server.a` (jnewbery)
...
- #13926 [Tools] ton-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like tond (MarcoFalke)
...
- #14521 qt, docs: Fix `ton-qt -version` output formatting (hebasto)
...
- #11625 Add tonApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libton_server` (laanwj)
...
- #15154 Configure: ton-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/ton-wallet (Sjors)
...
- #11526 Visual Studio build configuration for ton Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libton_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in ton-util-test.py (isghe)
...
- #12775 Integration of property based testing into ton Core (Christewart)
...
- #12246 Bugfix: Only run ton-tx tests when ton-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_ton` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_ton` (MarcoFalke)
...
- #14186 ton-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_ton.conf` message on startup if conf file exists (leishman)
...
- #12255 Update ton.service to conform to init.md (dongcarl)
...
- #14497 Add doc/ton-conf.md (hebasto)
...
- #15354 Add missing `ton-wallet` tool manpages (MarcoFalke)
...
- #15765 correct tonconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support ton folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/ton/).
