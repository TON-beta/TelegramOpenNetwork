...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " ton-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  ton-tx [options] <hex-tx> [commands]  Update hex-encoded ton transaction\n" +
            "or:     ton-tx [options] -create [commands]   Create hex-encoded ton transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded ton transaction
