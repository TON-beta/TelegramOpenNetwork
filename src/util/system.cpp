...
const char * const ton_CONF_FILENAME = "ton.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "tond -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //ton-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ton
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ton
    // Mac: ~/Library/Application Support/ton
    // Unix: ~/.ton
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ton";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ton";
#else
    // Unix
    return pathRet / ".ton";
#endif
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ton_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure ton Core copyright is not removed by accident
    if (copyright_devs.find("ton Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The ton Core developers";
    }
