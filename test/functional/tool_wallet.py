#!/usr/bin/env python3
# Copyright (c) 2018-2019 The ton Core developers
...
class ToolWalletTest(tonTestFramework):
    ...
    def ton_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/ton-wallet' + self.config["environment"]["EXEEXT"]
        args = ['-datadir={}'.format(self.nodes[0].datadir), '-regtest'] + list(args)
        return subprocess.Popen([binary] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)

    def assert_raises_tool_error(self, error, *args):
        p = self.ton_wallet_process(*args)
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `ton-wallet help` raises an error. Use `ton-wallet -help`.
